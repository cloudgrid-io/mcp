...
//   1. default (flag off) → grid_deploy carries NO openai/outputTemplate (the
...
//   3. MCP_APPS_WIDGETS=1 → grid_deploy DOES carry the outputTemplate (re-enabled).
...
const plugTpl = configs["grid_deploy"]?._meta?.["openai/outputTemplate"];
check("default: grid_deploy has NO openai/outputTemplate (text-first, no black square)", plugTpl == null);
...
process.stdout.write(String(configs["grid_deploy"]?._meta?.["openai/outputTemplate"] ?? "none"));
`;
const out = execFileSync(process.execPath, ["--input-type=module", "-e", child],
  { env: { ...process.env, MCP_APPS_WIDGETS: "1" }, encoding: "utf-8" }).trim();
check("MCP_APPS_WIDGETS=1: grid_deploy outputTemplate is restored", out === LIVE_RESULT_URI);
