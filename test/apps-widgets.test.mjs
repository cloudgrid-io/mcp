...
process.stdout.write(String(configs["grid_plug"]?._meta?.["openai/outputTemplate"] ?? "none"));
`;
const out = execFileSync(process.execPath, ["--input-type=module", "-e", child],
  { env: { ...process.env, MCP_APPS_WIDGETS: "1" }, encoding: "utf-8" }).trim();
check("MCP_APPS_WIDGETS=1: grid_plug outputTemplate is restored", out === LIVE_RESULT_URI);
