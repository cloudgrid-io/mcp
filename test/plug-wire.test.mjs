...
try {
  ...
  check("edit → prints the console line pointing at THEIR grid", /view it live in your grid/.test(a2.text) && a2.text.includes(consoleGridUrl("atomic")));
  check("edit → console_url is the grid-specific link (#355)", a2.structured.console_url === consoleGridUrl("atomic"));
  ...
  //    create — inspiration and app, web and local.
  {
    ...
    check("new deploy → prints the console line + grid-specific console_url (#355)", /view it live in your grid/.test(w.text) && w.structured.console_url === consoleGridUrl("acme"));
  }
