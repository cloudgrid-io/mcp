---
name: inventory
when: inventory management, stock tracker, warehouse, product stock, SKU manager — a persistent app that stores items (sku, name, qty, location, reorder_at) with quantity adjustments and a low-stock flag. Needs a database → runtime → local edition.
needs: database
deploy: runtime
editions: local
...
summary: Build a persistent Next.js + Mongo inventory / stock tracker on the grid — an `items` collection (sku, name, qty, location, reorder_at) with add / adjust-qty / delete and a low-stock flag. Edition-gate first, scaffold, put the app under services/web/, wire process.env.DATABASE_MONGODB_URL (legacy MONGODB_URL fallback) lazily, declare needs:{database:true} (not requires:), deploy async, poll to a live URL.
---

# Workflow: inventory

The user wants a items app that **remembers** its
...
same proven shape as `app-with-data`, with a `items` domain
(sku, name, qty, location, reorder_at) and a table view with add, per-row quantity adjustments (+/-), delete, and a low-stock flag.
...
2. Fetch the template for the Mongo wiring + CRUD shape:
   `grid_fetch("template", "inventory")`. It is a minimal, real Next.js +
   `mongodb`-driver app under `services/web/`: a lazy client in
   `services/web/lib/db.js`, an App-Router GET/POST/PATCH (adjust qty)/DELETE route on an `items` collection, a
