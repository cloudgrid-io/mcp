---
name: ticket-system
when: ticket system, support tickets, helpdesk, issue tracker, support queue — a persistent app that stores tickets (subject, requester, status, priority, body) with status changes. Needs a database → runtime → local edition.
needs: database
deploy: runtime
editions: local
...
summary: Build a persistent Next.js + Mongo support ticket queue on the grid — a `tickets` collection (subject, requester, status open/pending/closed, priority, body) with add / status-change / delete. Edition-gate first, scaffold, put the app under services/web/, wire process.env.DATABASE_MONGODB_URL (legacy MONGODB_URL fallback) lazily, declare needs:{database:true} (not requires:), deploy async, poll to a live URL.
---

# Workflow: ticket-system

The user wants a tickets app that **remembers** its
...
same proven shape as `app-with-data`, with a `tickets` domain
(subject, requester, status of open/pending/closed, priority, body) and a table view with add, per-row status changes, and delete.
...
2. Fetch the template for the Mongo wiring + CRUD shape:
   `grid_fetch("template", "ticket-system")`. It is a minimal, real Next.js +
   `mongodb`-driver app under `services/web/`: a lazy client in
   `services/web/lib/db.js`, an App-Router GET/POST/PATCH (status)/DELETE route on a `tickets` collection, a
