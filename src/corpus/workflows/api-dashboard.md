---
name: api-dashboard
when: API dashboard, API usage dashboard, API requests monitor — a persistent app that logs API requests and shows count / avg latency / error-rate metrics with a recent-requests table. Needs a database → runtime → local edition.
needs: database
deploy: runtime
editions: local
...
summary: Build a persistent Next.js + Mongo API dashboard on the grid — a `requests` collection (endpoint, status:number, ms:number, at) with a metrics header (count, avg ms, error rate), a record form, and a recent-requests table. Edition-gate first, scaffold, put the app under services/web/, wire process.env.DATABASE_MONGODB_URL (legacy MONGODB_URL fallback) lazily, declare needs:{database:true} (not requires:), deploy async, poll to a live URL.
---

# Workflow: api-dashboard

The user wants an API usage dashboard that **remembers** its request log — a
monitor whose recorded requests survive refresh and are shared across sessions.
That is a **runtime app** backed by the grid's shared Mongo, not a static page.
It is the same proven shape as `app-with-data`, with a `requests` domain
(`endpoint`, `status:number`, `ms:number`, `at`), a metrics header (request
count, average latency, error rate), a record form, and a recent-requests table
with per-row delete.
...
2. Fetch the template for the Mongo wiring + CRUD shape:
   `grid_fetch("template", "api-dashboard")`. It is a minimal, real Next.js +
   `mongodb`-driver request monitor under `services/web/`: a lazy client in
   `services/web/lib/db.js`, an App-Router GET/POST/DELETE route on a `requests`
   collection, a server-component page that reads the log, and a client dashboard
   that derives the metrics header (count, avg ms, error rate) and renders the
   record form + recent table.
