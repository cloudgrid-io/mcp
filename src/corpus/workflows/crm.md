---
name: crm
when: CRM, contacts, leads, sales pipeline, customer manager, deal tracker — a persistent app that stores and manages contacts/leads/customers by stage. Needs a database → runtime → local edition.
needs: database
deploy: runtime
editions: local
...
summary: Build a persistent Next.js + Mongo CRM on the grid — a `contacts` collection (name, email, company, stage, note) with add / restage / delete. Edition-gate first, scaffold, put the app under services/web/, wire process.env.DATABASE_MONGODB_URL (legacy MONGODB_URL fallback) lazily, declare needs:{database:true} (not requires:), deploy async, poll to a live URL.
---

# Workflow: crm

The user wants a contact manager / sales pipeline that **remembers** its
contacts — a CRM whose records survive refresh and are shared across sessions.
That is a **runtime app** backed by the grid's shared Mongo, not a static page.
It is the same proven shape as `app-with-data`, with a `contacts` domain
(name, email, company, stage of lead/qualified/customer, note) and a table view
with per-row stage changes.
...
2. Fetch the template for the Mongo wiring + CRUD shape:
   `grid_fetch("template", "crm")`. It is a minimal, real Next.js +
   `mongodb`-driver contact manager under `services/web/`: a lazy client in
   `services/web/lib/db.js`, an App-Router GET/POST/PATCH/DELETE route on a
   `contacts` collection, a server-component page, and a client table.
3. Adapt it (rename the collection, change fields, adjust the UI). **Read the DB
   ...
   - (Optional) fetch `grid_fetch("example", "crm")` for a richer filled
