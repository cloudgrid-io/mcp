---
name: event-board
when: event board, events listing, community events, meetup board — a persistent app that lists upcoming events (title, date, location, description) with add/delete, sorted by date. Needs a database → runtime → local edition.
needs: database
deploy: runtime
editions: local
...
summary: Build a persistent Next.js + Mongo event board on the grid — an `events` collection (title, date, location, description) shown as an upcoming list sorted by date, with add / delete. Edition-gate first, scaffold, put the app under services/web/, wire process.env.DATABASE_MONGODB_URL (legacy MONGODB_URL fallback) lazily, declare needs:{database:true} (not requires:), deploy async, poll to a live URL.
---

# Workflow: event-board

The user wants a community event board / events listing that **remembers** its
events — a board whose entries survive refresh and are shared across sessions.
That is a **runtime app** backed by the grid's shared Mongo, not a static page.
It is the same proven shape as `app-with-data`, with an `events` domain
(title, date, location, description) shown as an upcoming list sorted by date
ascending (soonest first), with add and delete.
...
2. Fetch the template for the Mongo wiring + CRUD shape:
   `grid_fetch("template", "event-board")`. It is a minimal, real Next.js +
   `mongodb`-driver event board under `services/web/`: a lazy client in
   `services/web/lib/db.js`, an App-Router GET/POST/DELETE route on an `events`
   collection, a server-component page that reads events sorted by date, and a
   client form + upcoming list.
3. Adapt it (rename the collection, change fields, adjust the UI). **Read the DB
   ...
   - The upcoming board sorts by `date` ascending both server-side (`.sort({ date:
     1 })`) and client-side after each add.
