---
name: directory
when: directory, business directory, member directory, listings directory — a persistent app that stores and shows searchable/filterable listings. Needs a database → runtime → local edition.
needs: database
deploy: runtime
editions: local
...
summary: Build a persistent Next.js + Mongo directory on the grid — an `entries` collection (name, category, url, description) with a searchable/filterable list plus add / delete. Edition-gate first, scaffold, put the app under services/web/, wire process.env.DATABASE_MONGODB_URL (legacy MONGODB_URL fallback) lazily, declare needs:{database:true} (not requires:), deploy async, poll to a live URL.
---

# Workflow: directory

The user wants a **directory** — a searchable, filterable list of listings that
**remembers** its entries: a business directory, member directory, or listings
directory whose records survive refresh and are shared across sessions. That is
a **runtime app** backed by the grid's shared Mongo, not a static page. It is the
same proven shape as `app-with-data`, with an `entries` domain (name, category,
url, description) and a list view with live search + category filtering and
per-row delete.
...
2. Fetch the template for the Mongo wiring + CRUD shape:
   `grid_fetch("template", "directory")`. It is a minimal, real Next.js +
   `mongodb`-driver directory under `services/web/`: a lazy client in
   `services/web/lib/db.js`, an App-Router GET/POST/DELETE route on an
   `entries` collection, a server-component page, and a client list with search +
   category filter.
