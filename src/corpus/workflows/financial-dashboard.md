...
when: financial dashboard, finance metrics, P&L, cashflow board — a persistent app that tracks income/expense entries by account and shows income/expense/net totals with a table. Needs a database → runtime → local edition.
needs: database
deploy: runtime
editions: local
...
summary: Build a persistent Next.js + Mongo financial dashboard on the grid — an `entries` collection (account, amount:number, type income/expense, at) with income/expense/net totals + table + add/delete. Edition-gate first, scaffold, put the app under services/web/, wire process.env.DATABASE_MONGODB_URL (legacy MONGODB_URL fallback) lazily, declare needs:{database:true} (not requires:), deploy async, poll to a live URL.
---

# Workflow: financial-dashboard

The user wants a financial dashboard / P&L / cashflow board that **remembers**
its numbers — income and expense entries whose records survive refresh and are
shared across sessions. That is a **runtime app** backed by the grid's shared
Mongo, not a static page. It is the same proven shape as `app-with-data`, with an
`entries` domain (account, amount:number, type of income/expense, at) and a
dashboard view: income / expense / net totals above a table with add and per-row
delete.
...
2. Fetch the template for the Mongo wiring + CRUD shape:
   `grid_fetch("template", "financial-dashboard")`. It is a minimal, real
   ...
   collection, a server-component page that computes income/expense/net totals,
   and a client board (totals cards + table + add form).
3. Adapt it (rename the collection, change fields, adjust the UI). **Read the DB
   ...
   - Keep the API route `export const dynamic = "force-dynamic"` so it always
     reads/writes live data.
