...
when: product catalog, product listings, catalog site, browse products (no checkout) — a persistent app that stores products and lets visitors browse them by category. Needs a database → runtime → local edition.
needs: database
deploy: runtime
editions: local
capabilities_note: persistent — needs a database (Mongo). Runtime app, async build, local edition only. Declare the canonical `needs: { database: true }`; the deployer injects DATABASE_MONGODB_URL (+legacy MONGODB_URL). No checkout / payments.
summary: Build a persistent Next.js + Mongo product catalog on the grid — a `products` collection (name, price, category, description, inStock) with a grid view, category filter, and admin add / delete. Edition-gate first, scaffold, put the app under services/web/, wire process.env.DATABASE_MONGODB_URL (legacy MONGODB_URL fallback) lazily, declare needs:{database:true} (not requires:), deploy async, poll to a live URL.
---

# Workflow: product-catalog

The user wants a product catalog / product-listings site that **remembers** its
products — a catalog whose records survive refresh and are shared across
sessions. That is a **runtime app** backed by the grid's shared Mongo, not a
static page. It is the same proven shape as `app-with-data`, with a `products`
domain (name, price, category, description, inStock) rendered as a grid with a
category filter and an admin add/delete section.

This is browse-only — **no checkout, no cart, no payments**. If the user wants to
sell (cart, checkout, Stripe), that is a different, larger build; say so.
...
2. Fetch the template for the Mongo wiring + CRUD shape:
   `grid_fetch("template", "product-catalog")`. It is a minimal, real
   Next.js + `mongodb`-driver catalog under `services/web/`: a lazy client in
   `services/web/lib/db.js`, an App-Router GET/POST/DELETE route on a `products`
   collection, a server-component page, and a client grid with a category filter
   and an admin add/delete form.
3. Adapt it (rename the collection, change fields, adjust the UI). **Read the DB
   ...
   - Keep the API route `export const dynamic = "force-dynamic"` so listings are
     always live.
...
Give the user the live catalog URL — that is the deliverable. To iterate, re-plug
the SAME entity so it updates the same URL. Keep it honest: async build,
local-edition only, credentials injected by the grid, browse-only (no checkout).
