...
when: revenue dashboard, sales dashboard, MRR/revenue view — a persistent app that records sales and shows total revenue, a per-product breakdown, and recent activity. Needs a database → runtime → local edition.
needs: database
deploy: runtime
editions: local
...
summary: Build a persistent Next.js + Mongo revenue dashboard on the grid — a `sales` collection (product, amount:number, at) rendered as a total-revenue figure, a per-product breakdown, and a recent-sales table with an add form. Edition-gate first, scaffold, put the app under services/web/, wire process.env.DATABASE_MONGODB_URL (legacy MONGODB_URL fallback) lazily, declare needs:{database:true} (not requires:), deploy async, poll to a live URL.
---

# Workflow: revenue-dashboard

The user wants a revenue / sales dashboard that **remembers** its sales — a view
whose numbers survive refresh and are shared across sessions. That is a
**runtime app** backed by the grid's shared Mongo, not a static page. It is the
same proven shape as `app-with-data`, with a `sales` domain
(`product`, `amount` as a number, `at` timestamp) and a dashboard view: a total
revenue figure, a per-product breakdown, a recent-sales table, and an add form.
...
2. Fetch the template for the Mongo wiring + CRUD shape:
   `grid_fetch("template", "revenue-dashboard")`. It is a minimal, real
   Next.js + `mongodb`-driver dashboard under `services/web/`: a lazy client in
   `services/web/lib/db.js`, an App-Router GET/POST/DELETE route on a `sales`
   collection, a server-component page that computes the total + per-product
   breakdown + recent table, and a client add-sale form.
3. Adapt it (rename the collection, change fields, adjust the UI). **Read the DB
   ...
   - Sales documents are `{ product: string, amount: number, at: Date }`; keep
     `amount` numeric so the total and breakdown sums are correct.
