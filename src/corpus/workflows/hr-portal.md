---
name: hr-portal
when: HR portal, employee portal, HR management, leave/PTO system — a persistent app where employees submit leave/PTO requests and admins approve them, with employee/admin auth. Needs a database + auth provider → runtime → local edition.
needs: database
deploy: runtime
editions: local
kind: blueprint
capabilities_note: persistent — needs a database (Mongo) + a provider SDK auth (Clerk/Auth0) with keys from the vault. Runtime app, async build, local edition only. Declare `needs: { database: true }` (deployer injects DATABASE_MONGODB_URL / legacy MONGODB_URL) and a `vault:` block for the auth keys. This is a BLUEPRINT — no app code ships; read AGENTS.md for the structure, then build.
summary: Build a persistent Next.js + Mongo HR/employee portal on the grid with employee/admin auth (Clerk/Auth0, keys from vault:) and a leave/PTO request + approval flow — `employees` + `leaveRequests` collections. This is a BLUEPRINT: fetch it, read AGENTS.md for the file tree + CloudGrid wiring (DATABASE_MONGODB_URL, vault keys, deploy), then build the app under services/web/. Edition-gate first, scaffold, declare needs:{database:true} + vault: auth keys (never requires:), deploy async, poll to a live URL.
---

# Workflow: hr-portal

The user wants an HR / employee portal — employees submit leave/PTO requests,
admins approve or reject them, gated by employee/admin auth. HR records and leave
requests must **remember** across refresh and be shared across users, so this is
a **persistent runtime app** backed by the grid's shared Mongo with a provider
SDK for auth — not a static page.

This is a **blueprint**. No app code ships. Fetch the template, **read its
`AGENTS.md` for the structure** (file tree, `employees` + `leaveRequests`
collections, CloudGrid wiring for DB + vault-backed auth keys, deploy flow), then
build the app following it.
...
to wire employee/admin auth, and the deploy steps.
...
1. Build the Next.js app under `services/web/` per `AGENTS.md` — the
   `employees` + `leaveRequests` collections, the leave-request + approval flow,
   the employee dashboard + admin queue, and the auth wiring (provider in the
   layout, middleware gate, server-side role checks).
