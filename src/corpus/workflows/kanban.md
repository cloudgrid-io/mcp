---
name: kanban
when: kanban board, task board, trello-style board, workflow columns — a persistent board whose cards move between columns and stick across refresh. Needs a database → runtime → local edition.
needs: database
deploy: runtime
editions: local
...
summary: Build a persistent Next.js + Mongo kanban board on the grid — a `cards` collection (title, column, order) with three columns and move-between-columns + add/delete. Edition-gate first, scaffold, put the app under services/web/, wire process.env.DATABASE_MONGODB_URL (legacy MONGODB_URL fallback) lazily, declare needs:{database:true} (not requires:), deploy async, poll to a live URL.
---

# Workflow: kanban

The user wants a kanban / trello-style board that **remembers** its cards — cards
that move between columns and survive refresh, shared across sessions. That is a
**runtime app** backed by the grid's shared Mongo, not a static page. It is the
same proven shape as `app-with-data`, with a `cards` domain (title, column of
todo/doing/done, order) and three columns with move-between-columns.
...
2. Fetch the template for the Mongo wiring + CRUD shape:
   `grid_fetch("template", "kanban")`. It is a minimal, real Next.js +
   `mongodb`-driver board under `services/web/`: a lazy client in
   `services/web/lib/db.js`, an App-Router GET/POST/PATCH/DELETE route on a
   `cards` collection (PATCH moves a card between columns), a server-component
   page, and a client board.
3. Adapt it (rename the collection, change the columns/fields, adjust the UI).
   **Read the DB from `process.env.DATABASE_MONGODB_URL`** (legacy
   ...
   - (Optional) fetch `grid_fetch("example", "kanban")` for a richer filled
