---
name: job-board
when: job board, careers page, job listings, hiring board, open roles — a persistent app that stores job postings (title, company, location, type, url) with a public list and post / delete. Needs a database → runtime → local edition.
needs: database
deploy: runtime
editions: local
...
summary: Build a persistent Next.js + Mongo job board on the grid — a `jobs` collection (title, company, location, type, url) with a public list and post / delete. Edition-gate first, scaffold, put the app under services/web/, wire process.env.DATABASE_MONGODB_URL (legacy MONGODB_URL fallback) lazily, declare needs:{database:true} (not requires:), deploy async, poll to a live URL.
---

# Workflow: job-board

The user wants a job board app that **remembers** its
...
same proven shape as `app-with-data`, with a `jobs` domain
(title, company, location, type, url) and a public list with post and delete.
