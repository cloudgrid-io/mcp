---
name: task-manager
when: task manager, to-do app (richer than the app-with-data starter), task list with due dates/priorities, complete-toggle + filter — a persistent task app that sticks across refresh. Needs a database → runtime → local edition.
needs: database
deploy: runtime
editions: local
...
summary: Build a persistent Next.js + Mongo task manager on the grid — a `tasks` collection (title, done, due, priority) with a complete-toggle, active/done filter, add/delete. Richer than the bare app-with-data to-do. Edition-gate first, scaffold, put the app under services/web/, wire process.env.DATABASE_MONGODB_URL (legacy MONGODB_URL fallback) lazily, declare needs:{database:true} (not requires:), deploy async, poll to a live URL.
---

# Workflow: task-manager

The user wants a task manager richer than the bare `app-with-data` to-do starter
— tasks with due dates and priorities, a complete-toggle, and an active/done
filter, all of which **stick** across refresh and are shared across sessions.
That is a **runtime app** backed by the grid's shared Mongo, not a static page.
It is the same proven shape as `app-with-data`, with a `tasks` domain (title,
done, due, priority of low/medium/high). (For the bare-minimum in-memory to-do,
use `app-with-data` instead.)
...
2. Fetch the template for the Mongo wiring + CRUD shape:
   `grid_fetch("template", "task-manager")`. It is a minimal, real Next.js +
   `mongodb`-driver task manager under `services/web/`: a lazy client in
   `services/web/lib/db.js`, an App-Router GET/POST/PATCH/DELETE route on a
   `tasks` collection (PATCH toggles done), a server-component page, and a client
   list with an active/done filter.
