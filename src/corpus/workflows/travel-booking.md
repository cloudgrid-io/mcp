---
name: travel-booking
when: travel booking portal, trip booking, flights/hotels booking, book a flight, book a hotel, travel reservations — a persistent app where users search and book flights/hotels and manage trips that survive refresh and are shared across sessions. Needs a database → runtime → local edition.
needs: database
deploy: runtime
editions: local
kind: blueprint
capabilities_note: "persistent — needs a database (Mongo). Runtime app, async build, local edition only. BLUEPRINT — ships structure + cloudgrid.yaml, not app code; read AGENTS.md and build. Declare the canonical `needs: { database: true }`; the deployer injects DATABASE_MONGODB_URL (+legacy MONGODB_URL). Payment/auth secrets via the `vault:` block → env vars. Trip-reminder cron is NOT yet deployable (platform issue #1543)."
summary: "Build a persistent Next.js + Mongo travel booking portal on the grid — `flights`, `hotels`, `bookings`, `trips` collections with search + availability logic, Stripe checkout, an auth provider, and a (pending) trip-reminder cron. This is a BLUEPRINT: fetch it, read AGENTS.md for the file tree + data model + CloudGrid wiring, then build the app under services/web/. Edition-gate first, scaffold, declare needs:{database:true} (not requires:), map secrets via vault:, deploy async, poll to a live URL."
---

# Workflow: travel-booking

The user wants a travel booking portal — search and book flights and hotels, pay,
and manage trips — that **remembers** its bookings across refresh and sessions.
That is a **runtime app** backed by the grid's shared Mongo, not a static page.
The proven runtime shape is the same as `app-with-data` (Next.js + lazy Mongo
client under `services/web/`), with a travel domain (`flights`, `hotels`,
`bookings`, `trips` with overbooking protection), Stripe checkout for paid
bookings, an auth provider for accounts, and a scheduled trip-reminder job.
...
static drop), that it needs the local edition, and that the trip-reminder **cron
...
1. Fetch the blueprint: `grid_fetch("template", "travel-booking")`. **Read its
...
4. **Keep the trip-reminder cron COMMENTED.** cron deploy is PENDING platform
   issue #1543 — `plug` rejects a `type: cron` service. Ship `web` alone; use the
...
vault env vars before deploying. Good for testing search/availability logic. Don't
require it.
...
only, credentials injected by the grid, and the trip-reminder cron pending #1543.
