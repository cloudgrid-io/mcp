...
when: property listings, real estate site, rentals/homes listings — a persistent site that stores and browses properties by location and price. Needs a database → runtime → local edition.
needs: database
deploy: runtime
editions: local
...
summary: Build a persistent Next.js + Mongo property listings site on the grid — a `properties` collection (title, price:number, location, beds:number, description) with a card grid, a location/price filter, a detail view, and an admin add form. Edition-gate first, scaffold, put the app under services/web/, wire process.env.DATABASE_MONGODB_URL (legacy MONGODB_URL fallback) lazily, declare needs:{database:true} (not requires:), deploy async, poll to a live URL.
---

# Workflow: property-listings

The user wants a real-estate / property listings site that **remembers** its
properties — listings that survive refresh and are shared across sessions. That
is a **runtime app** backed by the grid's shared Mongo, not a static page. It is
the same proven shape as `app-with-data`, with a `properties` domain
(title, price:number, location, beds:number, description) rendered as a card grid
with a location/price filter, a per-property detail view, and an admin add form.
...
2. Fetch the template for the Mongo wiring + CRUD shape:
   `grid_fetch("template", "property-listings")`. It is a minimal, real
   Next.js + `mongodb`-driver listings site under `services/web/`: a lazy client
   in `services/web/lib/db.js`, an App-Router GET/POST/DELETE route on a
   `properties` collection (GET supports `?location=` and `?maxPrice=` filters), a
   server-component page that reads + filters, and a client card grid with a
   detail view and admin add form.
