...
when: feature request board, feedback board, roadmap voting, idea board — a persistent app where users add requests and upvote them, each with a status. Needs a database → runtime → local edition.
needs: database
deploy: runtime
editions: local
...
summary: Build a persistent Next.js + Mongo feature request board on the grid — a `requests` collection (title, description, votes:number, status of open/planned/done) with list / upvote (atomic increment) / add. Edition-gate first, scaffold, put the app under services/web/, wire process.env.DATABASE_MONGODB_URL (legacy MONGODB_URL fallback) lazily, declare needs:{database:true} (not requires:), deploy async, poll to a live URL.
---

# Workflow: feature-request-board

The user wants a feature request / feedback board — a page where people add
requests, **upvote** the ones they care about, and each request carries a status
(open / planned / done). Votes and requests must **persist** and be shared across
users, so this is a **runtime app** backed by the grid's shared Mongo, not a
static page. It is the same proven shape as `app-with-data`, with a `requests`
domain (title, description, votes:number, status) and a voting board view.
...
- **Hosted edition (Claude Web / hosted MCP):** you CANNOT build a runtime app —
  ...
  plainly, offer a **static version** instead (a voting board whose state lives
  in memory only, lost on refresh), and STOP the runtime path here.
...
2. Fetch the template for the Mongo wiring + CRUD shape:
   `grid_fetch("template", "feature-request-board")`. It is a minimal, real
   Next.js + `mongodb`-driver board under `services/web/`: a lazy client in
   `services/web/lib/db.js`, an App-Router GET/POST/PATCH/DELETE route on a
   `requests` collection (PATCH does an atomic `$inc` upvote), a server-component
   page, and a client voting board.
3. Adapt it (rename the collection, change fields, adjust the UI). The domain is
   `requests { title, description, votes:number, status(open/planned/done) }`
   with list (votes desc) + upvote (increment) + add. **Read the DB from
   ...
   - Keep the vote increment atomic on the server (`$inc`), so concurrent upvotes
