---
name: blog-cms
when: blog, CMS, content site with posts, publishing platform, articles — a persistent app that stores and publishes posts (title, body, tags, published flag). Needs a database → runtime → local edition.
needs: database
deploy: runtime
editions: local
...
summary: Build a persistent Next.js + Mongo blog / CMS on the grid — a `posts` collection (title, slug, body, tags, published, createdAt) with a public list, a single-post page, and an admin create/delete screen. Edition-gate first, scaffold, put the app under services/web/, wire process.env.DATABASE_MONGODB_URL (legacy MONGODB_URL fallback) lazily, declare needs:{database:true} (not requires:), deploy async, poll to a live URL.
---

# Workflow: blog-cms

The user wants a blog, content site, or CMS that **remembers** its posts — a
publishing platform whose articles survive refresh and are shared across
sessions. That is a **runtime app** backed by the grid's shared Mongo, not a
static page. It is the same proven shape as `app-with-data`, with a `posts`
domain (title, slug, body, tags, published, createdAt): a public post list, a
single-post page at `/posts/[slug]`, and an admin screen to create and delete
posts (drafts stay off the public list).
...
2. Fetch the template for the Mongo wiring + CRUD shape:
   `grid_fetch("template", "blog-cms")`. It is a minimal, real Next.js +
   `mongodb`-driver blog under `services/web/`: a lazy client in
   `services/web/lib/db.js`, an App-Router GET/POST/DELETE route on a `posts`
   collection, a server-component list page, a single-post page at
   `app/posts/[slug]/page.js`, and an admin server page + client form.
3. Adapt it (rename the collection, change fields, adjust the UI). **Read the DB
   ...
   - **Keep DB-reading pages and the API route `export const dynamic =
     "force-dynamic"`** so they read/write live data instead of being
...
Give the user the live blog URL — that is the deliverable. To iterate, re-plug
