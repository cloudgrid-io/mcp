---
name: time-tracking
when: time tracking, timesheet, hours logger, billable hours — a persistent app that logs time entries (task, project, minutes, date) and totals hours per project. Needs a database → runtime → local edition.
needs: database
deploy: runtime
editions: local
...
summary: Build a persistent Next.js + Mongo time tracker on the grid — an `entries` collection (task, project, minutes:number, date) with add / list / delete and totals per project. Edition-gate first, scaffold, put the app under services/web/, wire process.env.DATABASE_MONGODB_URL (legacy MONGODB_URL fallback) lazily, declare needs:{database:true} (not requires:), deploy async, poll to a live URL.
---

# Workflow: time-tracking

The user wants a time tracker / timesheet / billable-hours logger that
**remembers** its entries — hours that survive refresh and are shared across
sessions. That is a **runtime app** backed by the grid's shared Mongo, not a
static page. It is the same proven shape as `app-with-data`, with an `entries`
domain (task, project, minutes, date), an add form, a list of entries, and a
per-project totals table.
...
2. Fetch the template for the Mongo wiring + CRUD shape:
   `grid_fetch("template", "time-tracking")`. It is a minimal, real Next.js +
   `mongodb`-driver time tracker under `services/web/`: a lazy client in
   `services/web/lib/db.js`, an App-Router GET/POST/DELETE route on an `entries`
   collection, a server-component page, and a client form that also renders
   per-project totals.
3. Adapt it (rename the collection, change fields, adjust the UI). The domain is
   `entries {task, project, minutes:number, date}`: add an entry, list entries,
   total minutes per project. **Read the DB from
   ...
   - The route handler is `export const dynamic = "force-dynamic"` so time
     entries are always read/written live.
